...
function updateTouristList()
{
    ...
    addGlobalStyle(
        ...
        '.abs { text-align: center; width: 100%; position: absolute; z-index: 100; padding-top: 79px; } \n' +
    ...
    if (window.location.href.indexOf("viewforum.php?f=17") > 0)
    {
        ...
        else
        {
            zDiv = $('<div></div>');
            zDiv.addClass('abs');
            $('#pagecontent').prepend(zDiv);
        }

        ///var $place = $('td.gensmall').eq(0);
    ...
    if (typeof(window.opera.version) == 'function' && window.opera.version() >= 9)
        document.addEventListener('DOMContentLoaded', updateTouristList, false);
    else
        document.addEventListener('load', updateTouristList, false);
})();
